...
# @click.command() # Not working for Python 3 on Mac OS
...
def main(env_id):
    ...
    if env_id == 'vishamC':
        ...
        # env = wrappers.Monitor(env,"/tmp/vishamC-1",force=True)
    ...
    for batch_size in batch_size_list:
        for alpha in alpha_list:
            for gamma in gamma_list:
                ...
                np.save("visham", theta)
                np.save("avg_visham_episode_rewards_lr_"+str(alpha)+"_gma_" +
                        str(gamma)+"_bs_"+str(batch_size), avg_episode_rewards)
...
if __name__ == "__main__":
    ...
    print(state_space)
    print(value_fun_lst)
    np.save("vishamc_state_space", state_space)
    np.save("vishamc_val_fun", value_fun_lst)




    # For Saving trajectries
    for epi in range(20):
        ob = env.reset()
        trajectory = []
        for i in itertools.count():
            ...
            if reached:
                ...
                episode_len = i
                env.done = False
                np.save("vishamC_traj_"+str(epi), trajectory)
                break
