# Plots comparing variation in learning curve for different values of hyperparameters Minibatch size
    ...
    def train(self, episodes_num=EPISODES_NUM):
        ...
        for episode in range(episodes_num):
            ...
            while True: #(done == False) and (episode_length < self.MAX_STEPS):
                ...
                #     self.summary_writer.add_summary(summary, episode_length)
...
if __name__ == '__main__':
    ...
    MINIBATCH_SIZES = [1,15,32,64,128]
    for MINIBATCH in MINIBATCH_SIZES:
        dqn = DQN('CartPole-v0',MINIBATCH)
        # dqn.load_model = True
        dqn.initialize_network()

        print("\nStarting training...\n")
        avg_100_steps, step_counts = dqn.train()
        np.save('saves/avg_100_steps_MINIBATCH_'+str(MINIBATCH), avg_100_steps)
        np.save('saves/step_counts_MINIBATCH_'+str(MINIBATCH), step_counts)
        ...
        np.save('saves/results_MINIBATCH_'+str(MINIBATCH), results)


    plt.clf()
    for MINIBATCH in MINIBATCH_SIZES:
        avg_100_steps = np.load('saves/avg_100_steps_MINIBATCH_'+str(MINIBATCH)+'.npy')
        x = np.arange(len(avg_100_steps))
        plt.plot(x, avg_100_steps,label=str(MINIBATCH))
    plt.title("Average total reward of last 100-episodes for different Minibatch sizes")
    plt.xlabel("Episodes")
    plt.ylabel("Average total reward of last 100-episodes")
    plt.legend(loc=0)
    # plt.xlim((0, dqn.EPISODES_NUM))
    plt.savefig('Avg_rewards_MINIBATCHs.png',dpi=300)

    plt.clf()
    for MINIBATCH in MINIBATCH_SIZES:
        step_counts = np.load('saves/step_counts_MINIBATCH_'+str(MINIBATCH)+'.npy')
        x = np.arange(len(step_counts))
        plt.plot(x, step_counts,label=str(MINIBATCH))
    plt.title("Episode Lengths for different Minibatch sizes")
    plt.xlabel("Episodes")
    plt.ylabel("Episode Length")
    plt.legend(loc=0)
    # plt.xlim((0, dqn.EPISODES_NUM))
    plt.savefig('Episode_lengths_MINIBATCHs.png', dpi=300)

    x = np.arange(100)
    plt.clf()
    for MINIBATCH in MINIBATCH_SIZES:
        results = np.load('saves/results_MINIBATCH_'+str(MINIBATCH)+'.npy')
        x = np.arange(len(results))
        plt.plot(x, results,label=str(MINIBATCH))
    plt.title("Learned agent episodes length for 100 plays for different Minibatch sizes")
    plt.xlabel("Episodes")
    plt.ylabel("Episode Length")
    plt.xlim((0, 100))
    plt.legend(loc=0)
    plt.savefig('Learned_Episode_lengths_MINIBATCHs.png', dpi=300)
