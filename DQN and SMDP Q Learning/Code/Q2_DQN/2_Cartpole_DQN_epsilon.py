# Plots comparing variation in learning curve for different values of hyperparameters a) Initial epsilon 
...
if __name__ == '__main__':
    ...
    EPSILIONS = [1,0.9,0.7,0.5]
    for EPSILON in EPSILIONS:
        dqn = DQN('CartPole-v0',EPSILON)
        # dqn.load_model = True
        dqn.initialize_network()

        print("\nStarting training...\n")
        avg_100_steps, step_counts = dqn.train()
        np.save('saves/avg_100_steps_epsilon_'+str(EPSILON), avg_100_steps)
        np.save('saves/step_counts_epsilon_'+str(EPSILON), step_counts)
        ...
        np.save('saves/results_epsilon_'+str(EPSILON), results)


    plt.clf()
    for EPSILON in EPSILIONS:
        avg_100_steps = np.load('saves/avg_100_steps_epsilon_'+str(EPSILON)+'.npy')
        x = np.arange(len(avg_100_steps))
        plt.plot(x, avg_100_steps,label=str(EPSILON))
    plt.title("Average total reward of last 100-episodes for different initial epsilon")
    plt.xlabel("Episodes")
    plt.ylabel("Average total reward of last 100-episodes")
    plt.legend(loc=0)
    # plt.xlim((0, dqn.EPISODES_NUM))
    plt.savefig('Avg_rewards_EPSILONs.png',dpi=300)

    plt.clf()
    for EPSILON in EPSILIONS:
        step_counts = np.load('saves/step_counts_epsilon_'+str(EPSILON)+'.npy')
        x = np.arange(len(step_counts))
        plt.plot(x, step_counts,label=str(EPSILON))
    plt.title("Episode Lengths for different initial epsilon")
    plt.xlabel("Episodes")
    plt.ylabel("Episode Length")
    plt.legend(loc=0)
    # plt.xlim((0, dqn.EPISODES_NUM))
    plt.savefig('Episode_lengths_EPSILONs.png', dpi=300)

    x = np.arange(100)
    plt.clf()
    for EPSILON in EPSILIONS:
        results = np.load('saves/results_epsilon_'+str(EPSILON)+'.npy')
        x = np.arange(len(results))
        plt.plot(x, results,label=str(EPSILON))
    plt.title("Learned agent episodes length for 100 plays for different initial epsilon")
    plt.xlabel("Episodes")
    plt.ylabel("Episode Length")
    plt.xlim((0, 100))
    plt.legend(loc=0)
    plt.savefig('Learned_Episode_lengths_EPSILONs.png', dpi=300)
