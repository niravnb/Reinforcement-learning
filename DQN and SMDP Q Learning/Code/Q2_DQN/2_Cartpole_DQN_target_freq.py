            ...
            while True: #(done == False) and (episode_length < self.MAX_STEPS):
                ...
                if len(self.replay_memory) >= self.MINIBATCH_SIZE:
                    ...
                    if total_steps % 100 == 0:
                        # self.summary_writer.add_summary(summary2, total_steps)
...
if __name__ == '__main__':
    ...
    TARGET_FREQS = [1,100,200,500,1000]
    for TARGET in TARGET_FREQS:
        dqn = DQN('CartPole-v0',TARGET)
        # dqn.load_model = True
        dqn.initialize_network()

        print("\nStarting training...\n")
        avg_100_steps, step_counts = dqn.train()
        np.save('saves/avg_100_steps_TARGET_'+str(TARGET), avg_100_steps)
        np.save('saves/step_counts_TARGET_'+str(TARGET), step_counts)
        ...
        np.save('saves/results_TARGET_'+str(TARGET), results)


    plt.clf()
    for TARGET in TARGET_FREQS:
        avg_100_steps = np.load('saves/avg_100_steps_TARGET_'+str(TARGET)+'.npy')
        x = np.arange(len(avg_100_steps))
        plt.plot(x, avg_100_steps,label=str(TARGET))
    plt.title("Average total reward of last 100-episodes for different Target update frequency")
    plt.xlabel("Episodes")
    plt.ylabel("Average total reward of last 100-episodes")
    plt.legend(loc=0)
    # plt.xlim((0, dqn.EPISODES_NUM))
    plt.savefig('Avg_rewards_TARGETs.png',dpi=300)

    plt.clf()
    for TARGET in TARGET_FREQS:
        step_counts = np.load('saves/step_counts_TARGET_'+str(TARGET)+'.npy')
        x = np.arange(len(step_counts))
        plt.plot(x, step_counts,label=str(TARGET))
    plt.title("Episode Lengths for different Target update frequency")
    plt.xlabel("Episodes")
    plt.ylabel("Episode Length")
    plt.legend(loc=0)
    # plt.xlim((0, dqn.EPISODES_NUM))
    plt.savefig('Episode_lengths_TARGETs.png', dpi=300)

    x = np.arange(100)
    plt.clf()
    for TARGET in TARGET_FREQS:
        results = np.load('saves/results_TARGET_'+str(TARGET)+'.npy')
        x = np.arange(len(results))
        plt.plot(x, results,label=str(TARGET))
    plt.title("Learned agent episodes length for 100 plays for different Target update frequency")
    plt.xlabel("Episodes")
    plt.ylabel("Episode Length")
    plt.xlim((0, 100))
    plt.legend(loc=0)
    plt.savefig('Learned_Episode_lengths_TARGETs.png', dpi=300)
